 ...
 /* loadnewJourney() {
  ...
  async saveJourney(journey:Journey) {// als Übergabeparameter eine Journey Übergeben
    ...
    if(journey.journeyID == null){
      await this.http.post("http://flock-1427.students.fhstp.ac.at/backend/public/newJourney", journey).toPromise().then((loadedData: Journey) => {
        console.log(loadedData);
      ...
      await this.http.post("http://flock-1427.students.fhstp.ac.at/backend/public/updateJourney", journey).toPromise().then((loadedData: Journey) => {
        console.log(loadedData);
